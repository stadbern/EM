...
# code for k-means algorithm to set means and covariance matrices for EM alogrithm

km.maison <- function(X, k){
  p <- ncol(X)  # number of parameters
  n <- nrow(X)  # number of observations
  
  Delta <- 1; iter <- 0; itermax <- 30 #Delta = measure of convergence
...
#' EM multivariate Gaussians plus uniform component
